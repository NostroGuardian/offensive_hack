...
<head>
    ...
    <title>Offensive Hack - XSS - 2 - Hard</title>
</head>
<body>
    <?php
    session_start();
...
<section class="chat">
    <div class="chat_frame">
        <div class="chat_frame_inner">
            ...
            $all_messages = $connection->query('SELECT * FROM `task_2_hard`')->fetchAll(PDO::FETCH_ASSOC);
            foreach ($all_messages as $m => $v){
                ...
                echo '<p class="chat_message" input='.htmlspecialchars($v['message']).'><span class="chat_nickname">'.$v['username'].': </span>'.htmlspecialchars($v['message'], ENT_QUOTES).'</p>';            }
            ?>
        </div>
    </div>
    <div class="chat_input">
        <div class="chat_input_inner">
            <form action="" method="post">
                <input class="chat_input_window" type="text" name="chat_message" placeholder="Enter your message..." required onfocus>
    ...
    if (isset($_POST['chat_btn'])) {
        ...
        if (!(trim($chat_message, $whitespaces))) {
            ...
            $query = 'INSERT INTO `task_2_hard` (username, message) VALUES (:chat_user, :chat_message)';
